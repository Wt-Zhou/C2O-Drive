#!/usr/bin/env python3
"""
SAC + CARLA Integration Script (Planner Mode)

Integrates SAC algorithm with CARLA simulation environment using the unified
Planner interface.

Features:
- Uses SACPlanner (following standard planner interface)
...
# SAC components
from c2o_drive.algorithms.sac import SACPlanner, SACConfig
...
class SACTrainer:
    """SAC Trainer for CARLA environment."""
    ...
    def _compute_q_statistics(self, state_features) -> Dict[str, float]:
        """Compute Q-value statistics (using critic network for SAC).
        ...
        """
        with torch.no_grad():
            q_values = self.planner.critic(state_features)
    ...
    def run_episode(
        ...
        # 2. Select trajectory using SAC policy
        state_features = self.planner._extract_state_features(state)
        with torch.no_grad():
            action_probs = self.planner.actor(state_features)
        ...
        episode_data = {
            ...
            'actor_loss': getattr(metrics, 'actor_loss', None) if metrics else None,
            'critic_loss': getattr(metrics, 'critic_loss', None) if metrics else None,
    ...
    def train(
        ...
        if self.verbose:
            print(f"\n{'='*70}")
            print(f" SAC Training (Planner Mode)")
            print(f"{'='*70}")
            print(f"  Episodes: {num_episodes}")
            print(f"  Max steps: {max_steps}")
            print(f"  Scenario: {scenario_name or 'default'}")
            print(f"  Action dim: {self.planner.config.action_dim}")
            print(f"{'='*70}\n")
        ...
        for episode in range(1, num_episodes + 1):
            ...
            if self.writer:
                ...
                self.writer.add_scalar('Train/Alpha', self.planner.alpha, episode)
            ...
            if self.verbose:
                ...
                print(f"Episode {episode}/{num_episodes} | "
                      ...
                      f"Avg: {avg_reward:.2f} | "
                      f"Steps: {stats['steps']} | "
                      f"Alpha: {self.planner.alpha:.3f} | "
                      f"Time: {episode_time:.1f}s {collision_str}")

            # Save checkpoint
            if episode % self.save_interval == 0:
                checkpoint_path = self.output_dir / f"sac_episode_{episode}.pt"
        ...
        # Save final model
        final_path = self.output_dir / "sac_final.pt"
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="SAC + CARLA Training Script (Planner Mode)",
    ...
    # Output settings
    parser.add_argument("--output-dir", type=str, default="outputs/sac_carla",
                       help="Output directory for checkpoints")
    parser.add_argument("--log-dir", type=str, default="logs/sac_carla",
...
def main():
    ...
    # Print configuration
    print(f"\n{'='*70}")
    print(f" SAC + CARLA Training (Planner Mode)")
    print(f"{'='*70}")
    print(f"\nCARLA Configuration:")
    ...
    try:
        ...
        # Save current model
        interrupt_path = output_dir / "sac_interrupted.pt"
