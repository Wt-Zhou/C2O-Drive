#!/usr/bin/env python3
"""
RCRL + CARLA Integration Script

Integrates Reachability-Constrained Reinforcement Learning (RCRL) algorithm
with CARLA simulation environment.

Features:
- Uses RCRLPlanner (following standard planner interface)
- Uses CarlaEnvironment (Gym standard interface)
- Forward reachable set computation for safety
...
class RCRLTrainer:
    """RCRL Trainer for CARLA environment."""
    ...
    def _compute_q_statistics(self, state_features) -> Dict[str, float]:
        """Compute Q-value statistics (using Q-network for RCRL).
    ...
    def run_episode(
        ...
        # 2. Select trajectory using RCRL policy (with safety constraints)
        ...
        for step in range(max_steps):
            ...
            # Track safety violations (if available in info)
            if step_result.info.get('safety_violation', False):
                safety_violations += 1
        ...
        episode_data = {
            ...
            'safety_violations': safety_violations,  # RCRL-specific metric
        }
    ...
    def train(
        ...
        if self.verbose:
            print(f"\n{'='*70}")
            print(f" RCRL Training")
            print(f"{'='*70}")
            print(f"  Episodes: {num_episodes}")
            print(f"  Max steps: {max_steps}")
            print(f"  Scenario: {scenario_name or 'default'}")
            print(f"  Constraint mode: {self.planner.config.constraint.mode}")
            print(f"  Action dim: {self.planner.config.get_n_actions()}")
            print(f"{'='*70}\n")
        ...
        for episode in range(1, num_episodes + 1):
            ...
            self.safety_violations.append(stats['safety_violations'])

            # Compute moving averages
            window = min(100, len(self.episode_rewards))
            avg_reward = np.mean(self.episode_rewards[-window:])
            avg_length = np.mean(self.episode_lengths[-window:])
            avg_violations = np.mean(self.safety_violations[-window:])
            ...
            if self.writer:
                ...
                self.writer.add_scalar('Train/SafetyViolations', stats['safety_violations'], episode)
                self.writer.add_scalar('Train/AvgViolations', avg_violations, episode)

            # Print progress
            if self.verbose:
                collision_str = "[COLLISION]" if stats['collision'] else ""
                violation_str = f"[{stats['safety_violations']} violations]" if stats['safety_violations'] > 0 else ""
                print(f"Episode {episode}/{num_episodes} | "
                      f"Reward: {stats['reward']:.2f} | "
                      f"Avg: {avg_reward:.2f} | "
                      f"Steps: {stats['steps']} | "
                      f"Time: {episode_time:.1f}s {collision_str} {violation_str}")

            # Save checkpoint
            if episode % self.save_interval == 0:
                checkpoint_path = self.output_dir / f"rcrl_episode_{episode}.pt"
        ...
        # Save final model
        final_path = self.output_dir / "rcrl_final.pt"
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="RCRL + CARLA Training Script",
    ...
    # RCRL-specific settings
    parser.add_argument("--constraint-mode", type=str, default="soft",
                       choices=["hard", "soft"],
                       help="Safety constraint mode (hard or soft)")
    ...
    # Output settings
    parser.add_argument("--output-dir", type=str, default="outputs/rcrl_carla",
                       help="Output directory for checkpoints")
    parser.add_argument("--log-dir", type=str, default="logs/rcrl_carla",
...
def main():
    ...
    if args.use_global_config:
        ...
        config.training.buffer_capacity = args.buffer_size

    # Set constraint mode
    config.constraint.mode = args.constraint_mode

    # Create planner using factory function
    if args.constraint_mode == "hard":
        planner = create_hard_constraint_planner(config)
    elif args.constraint_mode == "soft":
        planner = create_soft_constraint_planner(config)
    else:
        planner = RCRLPlanner(config)

    print(f"✓ Planner created")
    print(f"  Action space: {config.get_n_actions()} discrete actions")
    print(f"  State space: {config.network.state_dim} features")
    print(f"  Lattice: {len(config.lattice.lateral_offsets)} lateral × "
          f"{len(config.lattice.target_speeds)} speeds")
    print(f"  Constraint mode: {config.constraint.mode}\n")
    ...
    try:
        ...
        # Save current model
        interrupt_path = output_dir / "rcrl_interrupted.pt"
